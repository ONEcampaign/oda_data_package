...
def create_sectors_json() -> None:
    """Generate a json file with all sector configurations"""
    detailed_sectors = get_sector_groups()
    mapping = {}
    for detailed, broad in get_broad_sector_groups().items():
        mapping[broad] = {}

    for detailed, broad in get_broad_sector_groups().items():
        try:
            mapping[broad][detailed] = detailed_sectors[detailed]
        except KeyError:
            continue

    with open(config.OdaPATHS.settings / "sectors.json", "w") as f:
